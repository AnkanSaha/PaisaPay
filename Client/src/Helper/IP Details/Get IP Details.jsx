...
export default async function Get_IP_Details() {
    ...
    if(CacheStoredIP.data === null || CacheStoredIP.data === undefined || CacheStoredIP.data === '' || CacheStoredIP.status === false){
      ...
      await Cache.IP.saveCache("IP_Details", {IP_Address: CurrentIP, IP_Type: IP_Type, Details: IPDetails} ) // Store the IP Details in Cache Storage
      ...
      if(CacheStoredIP.data.IP_Address !== CurrentIP){
        const IPDetails = await IPDetailsExtractor(); // Get the IP Details
        await Cache.IP.updateCache("IP_Details", {IP_Address: CurrentIP, IP_Type: IP_Type, Details: IPDetails} ) // Store the IP Details in Cache Storage
