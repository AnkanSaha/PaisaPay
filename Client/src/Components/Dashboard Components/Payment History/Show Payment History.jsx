	...
	return (
		<>
			{isLoading === true ? (
				...
				<>
					<h1 className="text-center text-4xl font-mono font-bold mb-5 mt-[1.25rem]">Payment History of {Decoded_Account_Details.Name}</h1>
								...
								{ReduxState.TransactionDetails.Transactions.map((item, index) => {
									return (
										<tr key={index}>
											...
											<td>{Moment(item.TransactionDate).format('DD-MM-YY HH:mm:ss A')}</td>
											<td>
											...
											<td>
												<List>
													<ListItem key={index}>
														<ListIcon
															as={
																item.TransactionStatus === 'Transaction Failed' ? GiCrossMark : item.TransactionStatus === 'Processing' ? IoMdTimer : MdCheckCircle
															}
															color={
																item.TransactionStatus === 'Transaction Failed' ? 'red.500' : item.TransactionStatus === 'Processing' ? 'yellow.500' : 'green.500'
															}
														/>
