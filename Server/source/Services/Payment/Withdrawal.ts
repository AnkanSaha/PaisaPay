...
export const WithdrawalMoney = async (Request: Request, Response: ResponseInterface) => {
    try {
        ...
        if (AccountExistence.Information.Data[0].Balance < DecryptedData.Amount || AccountExistence.Information.Data[0].Balance === 0 || AccountExistence.Information.Data[0].Balance < 0) {
            Serve.JSON({
                status: false,
        ...
        // Deduct the amount from the account
         const DeductMoneyFromAccount = await MongoDB.ClientAccount.update([{ClientID: DecryptedData.ClientID}, {Email: DecryptedData.Email}], {Balance: AccountExistence.Information.Data[0].Balance - DecryptedData.Amount}, true); // Deduct the amount from the account
