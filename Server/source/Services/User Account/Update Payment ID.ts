...
export default async function UpdatePaymentID(Request: Request, Response: Response) {
	try {
		...
		const AccountDetails = await MongoDB.ClientAccount.findAndCount("AND", [
			{ ClientID: Decrypted_Data.ClientID },
			{ PhoneNumber: Decrypted_Data.PhoneNumber },
			{ Email: SmelledDetails.Email },
		]); // Find Account
		...
		if (AccountDetails.Data[0].AccountStatus !== "Active")
			return Serve.JSON({
				response: Response,
				status: false,
		...
		if ((await Compare(Decrypted_Data.TPIN, AccountDetails.Data[0].TransactionPIN)).isMatch === false)
			return Serve.JSON({
				response: Response,
				status: false,
				statusCode: StatusCodes.NOT_ACCEPTABLE,
		...
		if (SmelledDetails.Email === AccountDetails.Data[0].PaymentID)
			return Serve.JSON({
				response: Response,
				status: false,
				statusCode: StatusCodes.NOT_ACCEPTABLE,
		...
		const UpdateStatus = await MongoDB.ClientAccount.update(
			[{ ClientID: Decrypted_Data.ClientID }, { PhoneNumber: Decrypted_Data.PhoneNumber }, { Email: SmelledDetails.Email }],
			{ PaymentID: SmelledDetails.NewPaymentID },
			false
		); // Update Payment ID
